...
class TestSlackPluginMetadata:
    ...
    def test_get_metadata(self, slack_plugin):
        ...
        assert send_message_func.parameters["urgency"].choices == [
            "low",
            "normal",
            "high",
            "critical",
...
class TestSlackPluginInitialization:
    ...
    def test_initialize_without_credentials_fails(self, slack_plugin):
        ...
        with pytest.raises(
            PluginInitializationError,
            match="Either 'webhook_url' or 'bot_token' must be provided",
        ):
            slack_plugin.initialize(config)
    ...
    def test_initialize_with_invalid_timeout_fails(self, slack_plugin):
        ...
        with pytest.raises(
            PluginInitializationError, match="Timeout must be between 1 and 300 seconds"
        ):
            slack_plugin.initialize(config)
...
class TestSlackPluginSendMessage:
    ...
    def test_send_message_success(self, webhook_only_plugin, mock_requests):
        ...
        result = webhook_only_plugin.execute(
            "send_message",
            {
                "text": "Test message",
                "channel": "#test",
    ...
    def test_send_message_with_urgency_critical(
        self, webhook_only_plugin, mock_requests
    ):
    ...
    def test_send_message_supports_low_urgency_and_identity(
        self, initialized_plugin, mock_requests
    ):
        """Optional message presentation fields are included in the payload."""
        mock_response = Mock(status_code=200, text="ok")
        mock_requests.post.return_value = mock_response

        initialized_plugin.execute(
            "send_message",
            {
                "text": "Routine update",
                "username": "Playbook",
        ...
        assert payload["username"] == "Playbook"
        assert payload["icon_emoji"] == ":gear:"
        assert payload["text"] == ":information_source: Routine update"
...
class TestSlackPluginSendFile:
    ...
    def test_send_file_success(
        self, bot_token_only_plugin, mock_requests, mock_file_system
    ):
        ...
        result = bot_token_only_plugin.execute(
            "send_file",
            {
    ...
    def test_send_file_with_default_channel(
        self, bot_token_only_plugin, mock_requests, mock_file_system
    ):
    ...
    def test_send_file_nonexistent_file_fails(self, bot_token_only_plugin):
        ...
        with patch("playbook_slack.plugin.Path") as mock_path:
            ...
            with pytest.raises(PluginExecutionError, match="File not found"):
                bot_token_only_plugin.execute(
                    "send_file", {"file_path": "/nonexistent/file.txt"}
                )
    ...
    def test_send_file_api_error(
        ...
        with pytest.raises(
            PluginExecutionError, match="Slack API error: file_not_found"
        ):
            bot_token_only_plugin.execute("send_file", {"file_path": "/tmp/test.txt"})
...
class TestSlackPluginCreateChannel:
    ...
    def test_create_channel_name_taken_error(
        self, bot_token_only_plugin, mock_requests
    ):
        ...
        with pytest.raises(
            PluginExecutionError, match="Channel 'existing-channel' already exists"
        ):
            bot_token_only_plugin.execute(
                "create_channel", {"name": "existing-channel"}
            )

    def test_create_channel_invalid_name_error(
        self, bot_token_only_plugin, mock_requests
    ):
...
class TestSlackPluginIntegration:
    ...
    def test_complete_workflow_simulation(
        self, initialized_plugin, mock_requests, mock_file_system
    ):
        ...
        result1 = initialized_plugin.execute(
            "send_message",
            {
        ...
        result2 = initialized_plugin.execute(
            "create_channel",
            {
                "name": "deploy-alerts",
        ...
        result3 = initialized_plugin.execute(
            "send_file",
            {
