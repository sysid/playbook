...
class TestVariableDefinition:
    ...
    def test_given_valid_definition_when_creating_then_validates_successfully(self):
        ...
        definition = VariableDefinition(
            default="test", required=True, type="string", description="Test variable"
        )
    ...
    def test_given_int_type_with_choices_when_creating_then_validates_choices(self):
        ...
        definition = VariableDefinition(type="int", choices=[1, 2, 3])
    ...
    def test_given_int_type_with_invalid_choices_when_creating_then_raises_error(self):
        ...
        with pytest.raises(ValueError, match="not an integer"):
            VariableDefinition(type="int", choices=[1, "2", 3])

    def test_given_string_type_with_min_max_when_creating_then_raises_error(self):
        """Test string type with min/max raises error."""
        with pytest.raises(
            ValueError, match="min/max can only be used with int or float"
        ):
            VariableDefinition(type="string", min=1)
...
class TestVariableManager:
    ...
    def test_given_dict_with_templates_when_substituting_then_processes_recursively(
        self,
    ):
        """Test recursive substitution in dictionary."""
        data = {
            "title": "Deploy {{app}}",
            "config": {"timeout": "{{timeout}}", "env": "{{environment}}"},
    ...
    def test_given_numeric_constraints_when_validating_then_enforces_limits(self):
        ...
        variables = {"port": 80}
        definitions = {"port": VariableDefinition(type="int", min=1024, max=65535)}
    ...
    def test_given_template_string_when_extracting_variables_then_returns_variable_names(
        self,
    ):
...
""")
    ...
    def test_given_invalid_json_when_loading_then_raises_error(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
            ...
            try:
                with pytest.raises(
                    ConfigurationError, match="Cannot parse variable file"
                ):
