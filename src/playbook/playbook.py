...
class Playbook:
    ...
    def _run_step(self, step, existing_steps_by_name, resumed):
        ...
        if step.name in existing_steps_by_name:
            ...
            if step.repeatable is not True:
                print(
                    f"({italics('skipping already completed step')} '{step.preferred_name}')"
                )
                return
            else:
                print(
                    f"({italics('repeating existing step')} '{step.preferred_name}')\n\n"
                )
                print_title()
        ...
        while repeat is True:
            ...
            # handle negative response
                ...
                if step.skippable is True or resumed is True:
                    self._write_result(
                        step, plain_response, negative=True, reason="skipped"
                    )
                else:
                    if step.critical is True:
                        ...
                        reason = input("\t~> ").strip()
                        self._write_result(
                            step, plain_response, negative=True, reason=reason
                        )
            else:
    ...
    def _wait_for_response(self) -> bool:
        while True:
            ...
            if self._matches_any(
                response, ["yes+", "y", "yep", "yu+rp", "yeah+", "yar+", "yessir"]
            ):
    ...
    def _get_steps(self) -> List[Step]:
        ...
        for method_name, method in methods:
            ...
            if skippable is True and critical is True:
                raise Exception(
                    f"unsupported configuration for step '{step_name}': skippable steps cannot be critical"
                )
    ...
    def _get_ordered_methods(self):
        ...
        for c in classes:
            methods_by_class[c] = inspect.getmembers(
                c, lambda _: inspect.ismethod(_) or inspect.isfunction(_)
            )
        ...
        all_methods = inspect.getmembers(
            self, lambda _: inspect.ismethod(_) or inspect.isfunction(_)
        )
