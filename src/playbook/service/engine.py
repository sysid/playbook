...
from ..domain.models import (
    ...
    Runbook, RunInfo, NodeExecution, ManualNode, FunctionNode, CommandNode
)
from ..domain.ports import (
    Clock, ProcessRunner, FunctionLoader,
    RunRepository, NodeExecutionRepository, NodeIOHandler
)
...
class RunbookEngine:
    ...
    def resume_run(self, runbook: Runbook, run_id: int, start_node_id: Optional[str] = None) -> RunInfo:
    ...
    def resume_node_execution(
        ...
        try:
            ...
            if existing_execution.end_time and existing_execution.start_time:
                existing_execution.duration_ms = int(
                    (existing_execution.end_time - existing_execution.start_time).total_seconds() * 1000)
            ...
            # Handle exceptions
            existing_execution.end_time = self.clock.now()
            existing_execution.status = NodeStatus.NOK
            existing_execution.exception = str(e)
            ...
            return NodeStatus.NOK, existing_execution
    ...
    def execute_node_with_existing_record(
        ...
        # Get the current time for execution
        start_time = self.clock.now()
        ...
        try:
            ...
            # Create execution record for update
            ...
            return NodeStatus.NOK, execution
    ...
    def _execute_manual_node(self, node: ManualNode) -> NodeExecution:
        ...
        # If we have an IO handler, use it to get user decision
        if self.io_handler:
            approved = self.io_handler.handle_manual_prompt(node.id, node.name, node.prompt)
    ...
    def _execute_function_node(self, node: FunctionNode) -> NodeExecution:
        ...
        try:
            ...
            if self.io_handler and result:
                self.io_handler.handle_function_output(node.id, node.name, str(result))
    ...
    def _execute_command_node(self, node: CommandNode) -> NodeExecution:
        ...
        if self.io_handler and (stdout or stderr):
            self.io_handler.handle_command_output(node.id, node.name, stdout, stderr)
